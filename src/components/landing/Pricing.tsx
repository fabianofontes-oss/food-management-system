...
export function Pricing({ title, subtitle, plans }: PricingProps) {
  return (
    <section id="precos" className="relative py-24 sm:py-32 overflow-hidden">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-violet-100/40 via-transparent to-transparent dark:from-violet-900/20" />
            ...
            return (
              ...
              >
                <div className={`
                  ...
                  ${plan.highlighted 
                    ? 'bg-gradient-to-br from-violet-500 via-indigo-500 to-purple-500 p-[2px] shadow-2xl shadow-violet-500/30' 
                    : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-violet-300 dark:hover:border-violet-700 hover:shadow-xl'
                  }
                `}>
                  {/* Inner content */}
                  <div className={`
                  ...
                  `}>
                    ...
                    {plan.highlighted && (
                      <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-6 py-2 bg-gradient-to-r from-violet-500 to-indigo-500 text-white text-sm font-bold rounded-full shadow-lg shadow-violet-500/30 flex items-center gap-2">
                    ...
                    {/* Price */}
                    <div className="mb-8">
                      <span className={`text-5xl font-black ${plan.highlighted ? 'text-white' : 'bg-gradient-to-r from-violet-600 to-indigo-600 bg-clip-text text-transparent'}`}>
                        {plan.price}
                      </span>
                      {plan.period && (
                    ...
                    <ul className="space-y-4 mb-8">
                      {plan.features.map((feature, featureIndex) => (
                        <li key={featureIndex} className="flex items-start gap-3">
                          ...
                          `}>
                            <Check className={`w-3 h-3 ${plan.highlighted ? 'text-violet-400' : 'text-violet-600 dark:text-violet-400'}`} />
                          </div>
                    ...
                    <Link
                      ...
                      className={`
                        ...
                        ${plan.highlighted 
                          ? 'bg-gradient-to-r from-violet-500 to-indigo-500 text-white hover:shadow-lg hover:shadow-violet-500/30 hover:scale-[1.02]' 
