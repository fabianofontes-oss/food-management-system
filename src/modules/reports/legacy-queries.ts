...
export async function getSalesKPIs(
  ...
  const ordersArray = (orders ?? []) as Array<{
  ...
  const totalRevenue = ordersArray.reduce((sum, o) => sum + (o.total_amount || 0), 0)
...
export async function getDailyTrend(storeId: string, dateRange: DateRange): Promise<DailyTrend[]> {
  const supabase = await createClient()
  ...
  ;(orders as Array<{ created_at: string; total_amount: number | null }>).forEach((order) => {
...
export async function getDeliveryMetrics(storeId: string, dateRange: DateRange) {
  const supabase = await createClient()
  ...
  const deliveriesArray = deliveries as Array<{
    status: string | null
    created_at: string
    delivered_at: string | null
  }>

  const deliveredCount = deliveriesArray.filter((d) => d.status === 'delivered').length

  const deliveryTimes = deliveriesArray
    .filter((d) => d.status === 'delivered' && d.delivered_at)
    .map((d) => {
      const created = new Date(d.created_at).getTime()
      const delivered = new Date(d.delivered_at as string).getTime()
...
export function generateTopProductsCSV(products: TopProduct[]): string {
  ...
  const rows = products.map((p) => [p.product_name, p.quantity_sold.toString(), `R$ ${p.revenue.toFixed(2)}`])
